...
from utils.networks import ScannedRNN, ContinuousActorRNN, DiscreteActorRNN, DiscretePolicyVAE, EncoderWrapper, DiscretePolicyWithNextStateVAE
...
def train(config):
    ...
    vae = DiscretePolicyWithNextStateVAE(latent_dim=config.vae_latent_dim, Encoder_hidden_dim=32, action_dim=config.action_dim, state_dim=config.state_dim)
    ...
    def epoch_step(runner_state): 
        ...
        def train_one_batch(train_state_rng, batch):
            ...
            def loss_fn(params, batch, rng):
                ...
                pi, state, mu, log_var = vae.apply(params, (obs, done), rng)
                ...
                # jax.debug.print("done.shape={}, obs.shape={}, state.shape={}",done.shape,obs.shape,state.shape)
                Done=jnp.logical_not(done[:-1])
                # jax.debug.print("done[:][0]={}",done[:][0])
                (d1,d2)=Done.shape
                Done=jnp.expand_dims(Done,-1)
                Done=jnp.tile(Done,(1,1,obs.shape[-1]))
                dif=(obs[1:]-state[:-1])*Done
                # jax.debug.print("NonZero:{}",jnp.count_nonzero(Done))
                # jax.debug.print("Max:{} Min:{} NonZero:{}",jnp.max(obs),jnp.min(obs),jnp.count_nonzero(obs))
                loss = recon_loss + config.vae_kl_weight * kl_loss.mean() + config.vae_state_weight * jnp.mean(dif**2)
                return loss
    ...
    def encode(params, x, rng):
        pi, state, mu, log_var = vae.apply(params, x, rng)
